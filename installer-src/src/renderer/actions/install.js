...
async function cleanModulePatches(resourcesPath) {
    try {
        ...
        log(`[VibeCord] CleanModulePatches warning: ${err.message}`);
    }
}
...
function downloadFileAsync(url, destPath, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, { headers: { "User-Agent": "VibeCord-Installer/3.0" }, rejectUnauthorized: false }, (response) => {
...
const getJSON = phin.defaults({
    ...
    headers: { "User-Agent": "VibeCord-Installer/3.0", "Accept": "application/json" }
});
...
async function downloadDist() {
    ...
    try {
        await downloadFileAsync(assetUrl, tmpZip, (percent, downloaded, total) => {
            ...
            status.set(`Downloading VibeCord... (${dlMB}/${totalMB} MB)`);
        });
...
if (!fs.existsSync(patcherPath)) throw new Error('[VibeCord] patcher.js not found. Expected at: ' + primary);
require(patcherPath);
`;
...
async function injectShims(paths) {
    ...
    for (const resPath of paths) { // Now receives resources path from paths.js
        ...
        try {
            ...
            log("2. Configuring VibeCord loader...");
...
export default async function(paths) {
    try {
        ...
        await fs.mkdir(path.join(localAppData, "VibeCord"), { recursive: true });
        log("✅ Local AppData directory prepared");
        progress.set(MAKE_DIR_PROGRESS);
        lognewline("Downloading VibeCord package...");
