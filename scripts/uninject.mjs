/*
 * VibeCord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection VibeCord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par VibeCord
    if (existsSync(appDirPath)) {
        ...
        console.log("[VibeCord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[VibeCord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[VibeCord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[VibeCord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[VibeCord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[VibeCord] Aucune installation Discord avec VibeCord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que VibeCord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[VibeCord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[VibeCord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
