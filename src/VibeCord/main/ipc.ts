...
handleSync(IpcEvents.GET_GIT_HASH, () => VibeCord_GIT_HASH);
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.VibeCordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { VibeCordDir } = State.store;
    if (!VibeCordDir) return;

    const stats = await stat(VibeCordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(VibeCordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.VibeCordDir;
        return "ok";
    }
