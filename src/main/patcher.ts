...
console.log("[VibeCord] Starting up...");
...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[VibeCord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[VibeCord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "VibeCord");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[VibeCord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[VibeCord] Running in vanilla mode. Not loading VibeCord");
}

console.log("[VibeCord] Loading original Discord app.asar");
