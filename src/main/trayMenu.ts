...
function createVibeCordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "VibeCord",
            submenu: [
                {
                    label: "About VibeCord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update VibeCord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // No longer patching tray menu — VibeCord manages its own tray in patcher.ts
}
