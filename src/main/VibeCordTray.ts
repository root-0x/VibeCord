...
/**
 * Initializes the VibeCord system tray icon.
 ...
 */
export function initVibeCordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            console.log("[VibeCord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[VibeCord] Failed to create tray:", e);
        }
    });
}
