...
import "~plugins";
console.log("%c[VibeCord]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    ...
    titleSpan.textContent = "🔔 VibeCord Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A VibeCord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair VibeCord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of VibeCord.",
