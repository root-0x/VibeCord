...
export default definePlugin({
    name: "VibeCordHelper",
    description: "Main VibeCord plugin: fixes Discord crashes, adds UI options and manages internal commands.",
    authors: [
        Devs.thororen,
        EquicordDevs.nyx,
        EquicordDevs.Naibuu,
    ...
    patches: [
        ...
        // Show your own activity buttons because discord removes them for who knows why
        {
            ...
            predicate: () => settings.store.showYourOwnActivityButtons,
            replacement: {
                match: /.getId\(\)===\i.id/,
                replace: "$& && false"
            }
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (VC_SUPPORT_CHANNEL_IDS.includes(channelId) && !clicked) {
                return Alerts.show({
                    title: "VibeCord Support Channel Warning",
                    body: "Before asking for help. Check updates and if this issue is actually caused by VibeCord!",
                    confirmText: "VibeCord Support",
                    onConfirm() {
