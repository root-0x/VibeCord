...
function createTokenPreload(token: string): string {
    ...
    const script = `
// VibeCord MultiInstance — token preload
...
(function() {
    ...
    try {
        ...
        console.log("[VibeCordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[VibeCordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const win = new BrowserWindow({
            ...
            title: `VibeCord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `VibeCord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[VibeCordMI] setAppDetails failed:", err);
            }
        }
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[VibeCordMI] arrangeSplit error:", e);
    }
}
