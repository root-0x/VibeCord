...
// pour le token Discord stable. Quand VibeCord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît VibeCord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour VibeCord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[VibeCord] Module bloqué (inutile pour VibeCord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[VibeCord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[VibeCord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[VibeCord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    ...
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[VibeCord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[VibeCord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[VibeCord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[VibeCord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[VibeCord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[VibeCord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[VibeCord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[VibeCord] Impossible de patcher build_info.json:", e.message);
}
