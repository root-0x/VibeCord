@echo off
title VibeCord Installer - Build
cd /d "%~dp0"

echo.
echo  ================================
echo   VibeCord Installer - Build
echo  ================================
echo.
...
:: Build electron-builder -> VibeCord-Installer.exe dans ../release/installer/
echo.
...
:: Verification
if not exist "release\installer\VibeCord-Installer.exe" (
    echo.
    echo  [ERREUR] VibeCord-Installer.exe introuvable apres build.
    pause
    exit /b 1
)

for %%F in ("release\installer\VibeCord-Installer.exe") do (
    echo.
    echo  [OK] Build reussi ^!
    echo  Fichier : release\installer\VibeCord-Installer.exe  (%%~zF octets^)
    echo.
)
