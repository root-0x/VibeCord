# build-installer.ps1 — Build VibeCord-Installer.exe (Electron + electron-builder)
...
$OutExe    = Join-Path $OutDir "VibeCord-Installer.exe"

Write-Host ""
Write-Host "  [VibeCord] Building Electron installer..." -ForegroundColor Cyan
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  VibeCord-Installer.exe compile ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERREUR] VibeCord-Installer.exe introuvable apres compilation." -ForegroundColor Red
    exit 1
}
